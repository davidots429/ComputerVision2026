...
전체 1500장 중 1200장은 Train, 300장은 Test로 분할합니다. 데이터의 수가 상대적으로 적으므로 Validation은 학습 중 Train 데이터의 일부를 할당하거나 Test 데이터의 일부를 사용합니다. Yolo 계열 모델을 사용할 때는 1,200장의 검사 대상 이미지와 결함의 위치/종류가 적힌 Bounding Box 라벨링 텍스트 파일을 모두 사용하여 학습합니다. Anomalib 계열 모델을 사용할 때는 불량 데이터와 라벨링 정보는 전혀 사용하지 않습니다. 오직 1,200쌍의 데이터 중 정상 템플릿 이미지 1,200장만 사용하여 정상 패턴을 학습합니다.
